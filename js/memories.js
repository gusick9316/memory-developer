...
class MemoriesManager {
    ...
    async addMemory(title, author, year, month, day, imageFile) {
        const folderName = await window.githubAPI.generateUniqueFolderName('추억');
    ...
    async updateMemory(title, author, year, month, day, imageFile) {
        ...
        const folderName = memory.folderName || window.githubAPI.sanitizeFolderName(memory.title); // 기존 폴더명 사용
        const date = window.githubAPI.formatDate(year, month, day);
        ...
        if (imageFile && imageFile.size > 0) {
            const imageData = await window.githubAPI.fileToBase64(imageFile);
            const imageExtension = imageFile.name.split('.').pop();
            imagePath = `추억/${folderName}/image.${imageExtension}`;
    ...
    async handleDelete() {
        ...
        try {
            const folderName = this.editingMemory.folderName || window.githubAPI.sanitizeFolderName(this.editingMemory.title);
    ...
    async loadMemories() {
        ...
        while (retries > 0) {
            try {
                ...
                retries--;
                if (retries === 0) {
                    window.uiManager.showNotification('추억 불러오기에 실패했습니다.', 'error');
                    throw error;
                }
                console.log(`추억 불러오기 재시도 중... (${3 - retries}/3)`);
                await new Promise(resolve => setTimeout(resolve, 2000));
            }
        }
    }
