...
class StudentsManager {
    ...
    async handleSubmit() {
        ...
        try {
            ...
            stopLoadi    async addStudent(name, type, imageFile) {
        const folderName = await window.githubAPI.generateUniqueFolderName('학생목록');
    ...
    async updateStudent(name, type, imageFile) {
        ...
        const folderName = student.folderName || window.githubAPI.sanitizeFolderName(student.name); // 기존 폴더명 사용
        ...
        if (imageFile && imageFile.size > 0) {
            const imageData = await window.githubAPI.fileToBase64(imageFile);
            const imageExtension = imageFile.name.split('.').pop();
            imagePath = `학생목록/${folderName}/image.${imageExtension}`;
    ...
    async handleDelete() {
        ...
        try {
            const folderName = this.editingStudent.folderName || window.githubAPI.sanitizeFolderName(this.editingStudent.name);
    ...
    async loadStudents() {
        ...
        while (retries > 0) {
            try {
                ...
                retries--;
                if (retries === 0) {
                    window.uiManager.showNotification('학생목록 불러오기에 실패했습니다.', 'error');
                    throw error;
                }
                console.log(`학생목록 불러오기 재시도 중... (${3 - retries}/3)`);
