...
class DevelopersManager {
    ...
    async handleSubmit() {
        ...
        try {
            ...
            stopLoa    async addDeveloper(name, imageFile) {
        const folderName = await window.githubAPI.generateUniqueFolderName('개발자');
    ...
    async updateDeveloper(name, imageFile) {
        ...
        const folderName = developer.folderName || window.githubAPI.sanitizeFolderName(developer.name); // 기존 폴더명 사용
        ...
        if (imageFile && imageFile.size > 0) {
            const imageData = await window.githubAPI.fileToBase64(imageFile);
            const imageExtension = imageFile.name.split('.').pop();
            imagePath = `개발자/${folderName}/image.${imageExtension}`;
        ...
        // 폴더 삭제 로직 제거 - 기존 폴더명을 계속 사용하므로 불필요
    }
    ...
    async handleDelete() {
        ...
        try {
            const folderName = this.editingDeveloper.folderName || window.githubAPI.sanitizeFolderName(this.editingDeveloper.name);
    ...
    async loadDevelopers() {
        ...
        while (retries > 0) {
            try {
                ...
                retries--;
                if (retries === 0) {
                    window.uiManager.showNotification('개발자 정보 불러오기에 실패했습니다.', 'error');
                    throw error;
                }
                console.log(`개발자 정보 불러오기 재시도 중... (${3 - retries}/3)`);
