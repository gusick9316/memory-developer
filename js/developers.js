...
class DevelopersManager {
    ...
    async updateDeveloper(name, imageFile) {
        ...
        if (imageFile && imageFile.size > 0) {
            ...
            // 폴더명이 변경되었지만 이미지는 그대로인 경우 - 기존 이미지를 새 위치로 복사
            try {
                // 기존 이미지 데이터 가져오기
                ...
                await window.githubAPI.uploadImage(
                    imagePath,
                    oldImageContent,
                    `Move developer image: ${name}`
                );
            } catch (error) {
                console.error('Failed to copy image to new location:', error);
                // 이미지 복사 실패 시 기존 경로 유지
        ...
        // 폴더명이 변경된 경우 기존 폴더 삭제 (이미지 복사 후)
        if (oldFolderName !== newFolderName) {
            try {
                await window.githubAPI.deleteFolder(`개발자/${oldFolderName}`);
            } catch (error) {
                console.error('Failed to delete old folder:', error);
                // 기존 폴더 삭제 실패는 치명적이지 않으므로 계속 진행
            }
        }
    }
