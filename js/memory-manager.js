...
class MemoryManager {
    ...
    async loadAllMemories() {
        try {
            ...
            // DS-1부터 DS-20까지 모든 저장소 확인
            for (let i = 1; i <= 20; i++) {
                const repoName = `DS-${i}`;
                console.log(`${repoName} 저장소에서 추억 로딩 중...`);
                
                try {
                    // 저장소의 모든 폴더 목록 가져오기
                    const folders = await this.githubAPI.getRepositoryContents(repoName, '');
                    ...
                    if (folders && Array.isArray(folders)) {
                        for (const folder of folders) {
                            if (folder.type === 'dir') {
                                try {
                                    // 각 폴더에서 data.json 찾기
                                    const folderContents = await this.githubAPI.getRepositoryContents(repoName, folder.name);
                                    ...
                                    for (const file of folderContents) {
                                        if (file.name === 'data.json') {
                                            jsonFile = file;
                                        } else if (file.name.startsWith('image.')) {
                                            imageFile = file;
                                        }
                                    }
                                    ...
                                    if (jsonFile) {
                                        // JSON 데이터 로드
                                        const memoryData = await this.githubAPI.loadJSON(repoName, `${folder.name}/data.json`);
                                        
                                        if (memoryData) {
                                            // 추억 객체 생성
                                            const memory = {
                                                id: folder.name,
                                                title: memoryData.title || '제목 없음',
                                                author: memoryData.author || '작성자 미상',
                                                date: memoryData.date || '날짜 미상',
                                                repository: repoName,
                                                hasImage: !!imageFile,
                                            ...
                                            console.log(`추억 로드 성공: ${memory.title} (${repoName})`);
                                        }
                                    }
                                } catch (folderError) {
                                    console.warn(`폴더 ${folder.name} 처리 중 오류:`, folderError);
                                }
                            }
                        }
                    }
                } catch (repoError) {
                    console.warn(`저장소 ${repoName} 접근 실패:`, repoError);
                }
            }
            ...
            console.log(`총 ${this.memories.length}개의 추억을 로드했습니다.`);
...
class MemoryUI {
    ...
    // 개별 추억 요소 생성 (이미지 URL 생성 로직 개선)
    createMemoryElement(memory) {
        const div = document.createElement('div');
        div.className = 'memory-item fade-in';
        div.setAttribute('data-memory-id', memory.id);

        // 이미지 URL 생성 - 저장소와 폴더 구조에 맞게 정확히 생성
        let imageUrl = '';
        if (memory.hasImage && memory.imageFileName) {
            // GitHub Raw URL 형식: https://raw.githubusercontent.com/username/repository/main/folder/filename
            imageUrl = `https://raw.githubusercontent.com/${GITHUB_CONFIG.username}/${memory.repository}/main/${memory.id}/${memory.imageFileName}`;
        ...
        div.innerHTML = `
            ...
            ${memory.hasImage && imageUrl ? `
                <img src="${imageUrl}" 
                     alt="${this.escapeHtml(memory.title)}" 
                     class="memory-image" 
                     loading="lazy" 
                     onclick="showImageModal('${imageUrl}', '${this.escapeHtml(memory.title)}')" 
