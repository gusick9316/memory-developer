...
class GitHubAPI {
    ...
    // 중복되지 않는 폴더명 생성
    async generateUniqueFolderName(basePath) {
        let folderName;
        let attempts = 0;
        const maxAttempts = 10;

        do {
            folderName = this.generateRandomFolderName();
            attempts++;
            
            // 최대 시도 횟수 초과 시 타임스탬프 추가
            if (attempts >= maxAttempts) {
                folderName = this.generateRandomFolderName() + Date.now().toString().slice(-4);
                break;
            }
            
            // 폴더 존재 여부 확인
            try {
                await this.makeRequest(`${this.baseUrl}/contents/${basePath}/${folderName}`);
                // 폴더가 존재하면 계속 반복
            } catch (error) {
                // 404 에러면 폴더가 없다는 뜻이므로 사용 가능
